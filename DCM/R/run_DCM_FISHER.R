# TITLE: run_DCM_FISHER.R
# AUTHOR: Kevin O'Connor
# DATE CREATED: 9/20/18
# DATE MODIFIED: 9/20/18

# This code uses a modified p-value calculation based on the variance-
#  stabilizing Fisher transformation.

fisher <- function(x){
  return((1/2)*log((1+x)/(1-x)))
}

run_DCM_FISHER <- function(mat.1, 
                           mat.2, 
                           seed,
                           echo = FALSE, 
                           alpha = 0.05, 
  ...
  starttime = Sys.time()
    ...
    obs <- (1/(n1-3)+1/(n2-3))^(-1)*(fisher(corsm1*n_m1) - fisher(corsm2*n_m2))
    ...
    obss <- (1/(n1-3)+1/(n2-3))^(-1)*(fisher(corsm1*n_m1s) - fisher(corsm2*n_m2s))
