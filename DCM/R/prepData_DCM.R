prepData_DCM <- function(M1, 
  ...
  data.flags <- list()
	...
	p1 = nrow(M1)
	p2 = nrow(M2)
	preproc.steps <- append(preproc.steps, "Make sure datasets have same number of rows.")
	
	if(p1 != p2){
		print("Datasets must have the same number of rows.")
	  data.flags <- append(data.flags, "Datasets must have the same number of rows.")
		break
	}else{
		p = p1
	}
	...
	n1 = ncol(M1)
	n2 = ncol(M2)
  preproc.steps <- append(preproc.steps, "Make sure both datasets have 3 or more samples.")	
	if(n1 < 3 | n2 < 3){
		print("Must have at least 3 samples in each group.")
	  data.flags <- append(data.flags, "Both datasets must have at least 3 samples.")
		break
	}
	...
	if(length(killrows) > 0){
		...
		# Store data flags.
		if(length(na1) > 0){
		  data.flags <- append(data.flags, paste0("Found na values in the following rows of group 1: ", print(na1)))
		}
		if(length(na2) > 0){
		  data.flags <- append(data.flags, paste0("Found na values in the following rows of group 2: ", print(na2)))
		}
		if(length(lb1) > 0){
		  data.flags <- append(data.flags, paste0("Found too many small values in the following rows of group 1: ", print(lb1)))
		}
		if(length(lb2) > 0){
		  data.flags <- append(data.flags, paste0("Found too many small values in the following rows of group 2: ", print(lb2)))
		}
		if(length(ub1) > 0){
		  data.flags <- append(data.flags, paste0("Found too many large values in the following rows of group 1: ", print(ub1)))
		}
		if(length(ub2) > 0){
		  data.flags <- append(data.flags, paste0("Found too many large values in the following rows of group 2: ", print(ub2)))
		}
		if(length(var1) > 0 & strict == "high"){
		  data.flags <- append(data.flags, paste0("Found variance < 1/(100*n) in the following rows of group 1: ", print(var1)))
		}
		if(length(var2) > 0 & strict == "high"){
		  data.flags <- append(data.flags, paste0("Found variance < 1/(100*n) in the following rows of group 2: ", print(var2)))
		}
	}else{
		idcs = 1:p
