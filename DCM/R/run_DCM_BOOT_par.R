...
run_DCM_BOOT <- function(mat.1, 
  ...
  starttime = Sys.time()
	...
	while(difference > 0 & length(A) > 5){
	  ...
	  obs.test.stats <- 1:p
	  obs.test.stats[-A] <- colMeans(A1 %*% t(y1) - A2 %*% t(y2))
	  obs.test.stats[A] <- colMeans(A1 %*% t(A1) - A2 %*% t(A2))
	  
    start.time <- Sys.time()
    # Get bootstrapped distribution of test statistics.
    boot.iter <- 1000
    excedences <- mclapply(1:boot.iter, FUN=function(i){
      ...
      A1.samp <- stdize(A1.samp)
      A2.samp <- stdize(A2.samp)
      ...
      return(as.numeric(obs.test.stats < test.stats))
    }, mc.cores=n.cores) %>% Reduce(f='+')
