...
# Mean Differential Correlation

In this section we will track the mean differential correlation of the active set, $A$, at different points in the algorithm. We begin by investigating the initialization performed during DCM for the RNAseq data.
```{r mean_diff_cor1}
load("/Users/kevinoconnor/Documents/Research/DCM/Test/2018_08_29 11:33:06/No QR/Debug_Output/Initialization.RData")

# Calculate difference of correlation matrices.
cor.LA <- MAM.LA.std %*% t(MAM.LA.std)
cor.LB <- MAM.LB.std %*% t(MAM.LB.std)
diff.cor <- cor.LA - cor.LB

# Get mean differential correlation of active set at each iteration.
init.diff.cor <- c()
for(i in 1:length(initialization$sets)){
  A <- initialization$sets[[i]]
  init.diff.cor <- c(init.diff.cor, mean(diff.cor[A,A]))
}
plot(init.diff.cor, main="Mean Differential Correlation during Initialization", ylab="Mean Differential Correlation", xlab="Iteration")
```

The plot above tracks the following at each iteration,
\[ \frac{1}{|A|^2}\sum\limits_{i\in A}\sum\limits_{j \in A} (\mathbb{R}_1 - \mathbb{R}_2)_{ij} \]
as an aggregate measure of the differential correlation of the set $A$. Interestingly, we see that a maximum is reached around the 50th iteration, and a steady decreasing trend is observed until the end of initialization.

Now we will look at the mean differential correlation during the iterative testing portion of the algorithm.

```{r mean_diff_cor2}
load("/Users/kevinoconnor/Documents/Research/DCM/Test/2018_08_29 11:33:06/No QR/Debug_Output/DCM_1.RData")

diff.cor.v <- c()
for(i in 1:(length(DCM$it_sets)-1)){
  A <- DCM$it_sets[[i]]
  diff.cor.v <- c(diff.cor.v, mean(diff.cor[A,A]))
}

plot(diff.cor.v, main="Mean Differential Correlation during Iterative Testing", ylab="Mean Differential Correlation", xlab="Iteration")
```

Note that we see a large drop off in mean differential correlation from where the algorithm was initialized. This indicates that the iterative testing procedure is not selecting elements of the DC set appropriately. 


# P-values

Next we take a look at the p-values that were observed during this test.

```{r p_vals1}
for(i in c(1, 5, 10)){
  hist(DCM$it_p_vals[[i]], main=paste0("P-values from Iteration ", i))
}
```

Clearly an error has occurred at some point in the p-value computation as this distribution is basically nonsense.
